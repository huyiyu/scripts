...
public class ExampleDualStackNameResolver extends NameResolver {
            ...
            ImmutableMap.<String, List<List<SocketAddress>>>builder()
                    .put("lb.example.grpc.io",
                            Arrays.stream(SERVER_PORTS)
                                    .mapToObj(ExampleDualStackNameResolver::getLocalAddrs)
    ...
    private void resolve() {
        ...
        try {
            ...
            for (List<SocketAddress> endpoint : Objects.requireNonNull(addresses)) {
            ...
            this.listener.onResult(ResolutionResult.newBuilder().setAddressesOrError(StatusOr.fromValue(eagList)).build());
        } catch (Exception e) {
